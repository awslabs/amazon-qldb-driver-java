...
public class MockQldbSessionClient implements QldbSessionV2AsyncClient {
    ...
    @Override
    public CompletableFuture<Void> sendCommand(SendCommandRequest sendCommandRequest, Publisher<CommandStream> requestStream,
                                               SendCommandResponseHandler asyncResponseHandler) {
        ...
        CompletableFuture<Void> executeFuture = CompletableFuture.runAsync(() -> {
            while (resultQueue.peek() != null) {
                ...
                if (holder.exception != null) {
                    ...
                    final SdkPublisher<ResultStream> sdkPublisher =
                        SdkPublisher.adapt(resultToDeliver.toFlowable(BackpressureStrategy.ERROR));
